...
列表和文字可以在既有 `[起点:终点]` 后再写步长；三段都可省略，结果是新值：

```cnp
设 数字列表 = [0, 1, 2, 3, 4, 5]
打印(数字列表[::2])   # [0, 2, 4]：取位置 0、2、4
打印(数字列表[::-1])  # [5, 4, 3, 2, 1, 0]：从后向前取
```

步长必须是非零整数。负步长表示从后向前；完整边界见
[语义约定](docs/spec/00-语义约定.md)。
...
| — | 否则如果/默认参数/关键字参数/成员方法/自增 | ✅ v0.5.0 |
| — | 输入(询问)/插值串(@)/多返回值 | ✅ v0.6.0 |
...
**学习 CNplus** → [Wiki 教程站](https://wiki.cnplus.org)：零基础起步，12 章循序渐进，
配练习题和可运行示例。

本仓库只保留面向实现者与贡献者的规范文档：
