...
def _找配对右花括号(文本: str, 左位置: int) -> int:
    """找插值外层的配对右花括号，忽略字符串里的花括号。"""
    深度 = 1
    引号: str | None = None
    转义 = False
    for 位置 in range(左位置 + 1, len(文本)):
        字 = 文本[位置]
        if 引号 is not None:
            if 转义:
                转义 = False
...
def _解码字符串内容(原文本: str) -> str:
    """与 lexer 字符串转义保持一致，用于格式串中的普通文字片段。"""
    结果: list[str] = []
    i = 0
    转义 = {"n": "\n", "t": "\t", "\\": "\\", '"': '"', "'": "'"}
    while i < len(原文本):
        if 原文本[i] == "\\" and i + 1 < len(原文本):
            结果.append(转义.get(原文本[i + 1], 原文本[i + 1]))
            i += 2
        else:
            结果.append(原文本[i])
            i += 1
...
class 解析器:
    ...
    def _声明(self) -> 语句:
        ...
        if self._看种() is 种类.逗号:
            ...
            while self._匹配(种类.逗号):
                后一个 = self._期望(种类.标识符, "变量名")
                名们.append(后一个.文本)
                名跨们.append(后一个.跨)
            self._期望(种类.赋值, "等号 =")
            值 = self._表达式()
            self._行尾()
            return 多重声明语句(名们=tuple(名们), 值=值,
                              跨=self._跨到此(起), 名跨们=tuple(名跨们))
        self._期望(种类.赋值, "等号 =")
        值 = self._表达式()
        self._行尾()
    ...
    def _导入(self) -> 语句:
        ...
        if self._看种() is 种类.作为:
            ...
            别名跨 = 名.跨
        self._行尾()
        return 导入语句(模块=cast(str, 模块词元.值), 别名=别名,
                      跨=self._跨到此(起), 别名跨=别名跨,
                      模块跨=模块词元.跨)
    ...
    def _表达式语句(self) -> 语句:
        ...
        if isinstance(表, 索引访问) and self._看种() is 种类.赋值:
            ...
            return 索引赋值语句(对象=表.对象, 下标=表.下标, 值=值,
                           跨=self._跨到此(起))
        # 索引复合赋值保留为 AST 语义；后端必须让对象、下标各求值一次。
        if isinstance(表, 索引访问) and self._看种() in (
            ...
            右 = self._表达式()
            self._行尾()
            return 索引赋值语句(对象=表.对象, 下标=表.下标, 值=右,
                           跨=self._跨到此(起), 复合=运)
        # 成员赋值：自己.字段 = 值 / 对象.字段 = 值（D-036 类）
        if isinstance(表, 成员访问) and self._看种() is 种类.赋值:
            self._吃()
            值 = self._表达式()
            self._行尾()
            return 成员赋值语句(对象=表.对象, 属性=表.属性, 值=值,
                            跨=self._跨到此(起))
        # 成员复合赋值保留为 AST 语义；后端必须让接收者只求值一次。
        if isinstance(表, 成员访问) and self._看种() in (
                种类.加赋, 种类.减赋, 种类.乘赋, 种类.除赋):
            符 = self._吃().种
            运 = {种类.加赋: 二元运算.加, 种类.减赋: 二元运算.减,
                 种类.乘赋: 二元运算.乘, 种类.除赋: 二元运算.除}[符]
            右 = self._表达式()
            self._行尾()
            return 成员赋值语句(对象=表.对象, 属性=表.属性, 值=右,
                            跨=self._跨到此(起), 复合=运)
        self._行尾()
    ...
    def _遍历旧语序(self) -> 语句:
        """对于 x 在 y ≡ 遍历 y 每个 x（D-021 旧语序复活为别名）。

        历史：v0.4.2 前的主写法（直译 for x in y），D-021 改语序时
        删过头。2026-08-25 拍板作为别名保留——解析成同一个
        遍历语句 节点，后端零改动。
        """
        起 = self._吃().跨  # 对于
        变量 = self._期望(种类.标识符, "变量名")
    ...
    def _函数(self) -> 语句:
        ...
        try:
            ...
            # 编辑器里函数头常先于函数体出现；保留半成品 AST 供 LSP 使用。
            主体 = ()
            空体跨 = self.当前.跨
        声明跨 = self._跨到此(起)
        if 空体跨 is not None:
    ...
    def _类(self) -> 语句:
        """类 名 → 类声明（方案 C，无继承，D-036）。

        类体就是一个普通缩进块；块里「初始化(...)」是构造
        （括号参数自动成字段），「函数 ...」是方法。通过
        _语句 里的 初始化 分支拿到构造声明再分类。
        """
        起 = self._吃().跨  # 类
        名词元 = self._期望(种类.标识符, "类名")
        方法们: list[函数声明] = []
        初始化声明: 函数声明 | None = None
        for s in self._块():
            ...
            if isinstance(s, 函数声明) and s.名 == "初始化":
                ...
                self.袋.报告(CN0202_意外词元,
                           "类体里只能写「初始化」或「函数」",
                           s.跨,
                           解释="类的身体由构造和方法组成，别的代码要写进方法里",
                           提示="把这段代码写进某个方法，或挪到类外面")
        return 类声明(名=名词元.文本, 方法们=tuple(方法们),
                    初始化=初始化声明, 跨=self._跨到此(起), 名跨=名词元.跨)

    def _初始化(self) -> 语句:
        """初始化(参数...) [块体] —— 构造方法，仅类体内合法。

        括号参数自动转存为字段；块体可省略（直接换行）。
        """
        起 = self._吃().跨  # 初始化
        self._期望(种类.左括号, "左括号 (")
        形参: list[str] = []
        ...
        self._期望(种类.右括号, "右括号 )")
        主体起 = self.当前.跨.止
        # 块体可省略：换行/退缩/文件尾都算「只有参数」
        主体: tuple[语句, ...] = ()
        存位 = self.位
        while self._看种() is 种类.换行:
            self._吃()
        if self._看种() is 种类.缩进:
            self.位 = 存位
            主体 = self._块()
        else:
            self.位 = 存位  # 把吞掉的换行还给外面（类体块会再匹配）
        声明跨 = self._跨到此(起)
        主体跨 = 跨度(主体起, 声明跨.止) if 主体 else None
        return 函数声明(名="初始化", 形参=tuple(形参), 主体=主体,
                      跨=声明跨, 名跨=起, 主体跨=主体跨,
                      形参跨们=tuple(形参跨们),
                      默认值们=tuple(默认值们))

    def _跳到行尾(self) -> None:
        while self._看种() not in (种类.换行, 种类.文件尾, 种类.退缩):
            self._吃()
    ...
    def _索引(self, 对象: 表达式) -> 表达式:
        ...
        if 是切片:
            return 切片访问(对象=对象, 起=起, 止=止, 步长=步长,
                           跨=合并跨度(对象.跨, 右.跨))
        if 起 is None:
            # a[]：什么都没写。报错并造个错误表达式顶位
            self.袋.报告(CN0203_期望表达式,
                       "方括号里需要一个下标（比如 0）或 切片（比如 1:3）",
                       右.跨,
                       解释="方括号是「取第几个」的意思，里面要写位置",
                       提示="例如：名单[0] 或 名单[1:3]")
            起 = 错误表达式(跨=右.跨, 说明="空下标")
        return 索引访问(对象=对象, 下标=起, 跨=合并跨度(对象.跨, 右.跨))
    ...
    def _前缀(self) -> 表达式:
        ...
        if 种 is 种类.插值符:
            ...
            return self._解析格式串(串.文本[1:-1], 合并跨度(t.跨, 串.跨))
        if 种 in (种类.真, 种类.假):
            self._吃()
    ...
    def _解析格式串(self, 文本: str, 跨: 跨度) -> 表达式:
        ...
        while i < n:
            c = 文本[i]
            if c == "\\" and i + 1 < n:
                缓.extend((c, 文本[i + 1])); i += 2; continue
            ...
            if c == "{":
                ...
                if j == -1:
                    ...
                    break
                if 缓:
                    部分.append(_解码字符串内容("".join(缓))); 缓 = []
                原表达式源 = 文本[i + 1 : j]
                前空白 = len(原表达式源) - len(原表达式源.lstrip())
                表达式源 = 原表达式源.strip()
                起偏移 = 跨.起.偏移 + 2 + i + 1 + 前空白
                本地起偏移 = 起偏移 - self.源.基位置.偏移
                子源 = 源文件(表达式源, self.源.文件名,
                             基位置=self.源.位置于(本地起偏移))
                子 = 解析器(子源, self.袋)
                子表达式 = 子._表达式()
                子._跳过换行()
                if not 子._到头():
                    self.袋.报告(CN0203_期望表达式,
                               f"插值里一个值后面还有多余内容：{子._描述当前()}",
                               子.当前.跨,
                               解释="每对花括号里必须恰好是一个完整表达式",
                               提示="删除多余内容，或用运算符把它连成一个表达式")
                部分.append(子表达式)
                i = j + 1
                continue
            缓.append(c); i += 1
        if 缓:
            部分.append(_解码字符串内容("".join(缓)))
