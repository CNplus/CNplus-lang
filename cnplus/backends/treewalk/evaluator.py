...
class 树遍历后端(后端):
    ...
    def _实例化(self, 类: 类值, 实参: list,
              关键字: list[tuple[str, object]], 跨, 环: 环境) -> object:
        ...
        if 初始化声明 is None:
            if 实参 or 关键字:
                raise 运行时错误(CN0306_实参数量不符,
                              f"{类.名} 没有写「初始化」，不需要参数，给了 {len(实参) + len(关键字)} 个",
                              跨,
    ...
    def _调用(self, e: 调用表达式, 环: 环境) -> object:
        # 成员方法调用先求值接收者，再判断是否是 CNplus 内置点方法；
        # 所有实参仍须从左到右各求值一次，之后才检查参数合同。
        点内置项: _内置 | None = None
        对象值 = None
        ...
        if isinstance(被调, _内置):
            ...
            try:
                ...
                消息 = (f"调用 {被调.名} 出错：{ex}"
                      if 点内置项 is not None else str(ex))
                raise 运行时错误(CN0308_下标越界, 消息, e.跨,
                              解释="要弹出的位置不在列表范围内",
                ...
                消息 = (f"调用 {被调.名} 出错：{ex}"
                      if 点内置项 is not None else str(ex))
                raise 运行时错误(CN0303_类型不匹配, 消息, e.跨) from None
    ...
    def _绑定实参(self, 声明, 实参: list,
              关键字: list[tuple[str, object]], 跨) -> "环境":
        """按 CNplus 合同绑定自定义函数、方法或初始化参数。"""
        形参 = 声明.形参
        默认们 = 声明.默认值们 or tuple(None for _ in 形参)
        局部 = 环境()
        if 实参 and 关键字:
            raise 运行时错误(CN0306_实参数量不符,
                          f"{声明.名} 的一次调用不能混用位置参数和「名字 = 值」参数", 跨,
                          解释="自定义函数的一次调用只能全部按位置传值，或全部写成「名字 = 值」",
                          提示="统一改成一种写法")
        if len(实参) > len(形参):
            raise 运行时错误(CN0306_实参数量不符,
