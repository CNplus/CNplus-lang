    ...
    def _取切片(self, e: 切片访问, 环: 环境) -> object:
        ...
        if not isinstance(对象, (list, str)):
            raise 运行时错误(CN0303_类型不匹配,
                          ...
                          提示="检查这个东西的类型")
        起 = self._切片整数(e.起, 环, "起点")
        止 = self._切片整数(e.止, 环, "终点")
        步 = self._切片整数(e.步, 环, "步长")
        if 步 == 0:
            assert e.步 is not None
            raise 运行时错误(CN0303_类型不匹配, "切片步长不能是 0", e.步.跨,
                          解释="步长是每次往前或往后走几格，写 0 就永远走不动",
                          提示="改成 1、2 或 -1 这类非零整数")
        return 对象[slice(起, 止, 步)]

    def _切片整数(self, 节点, 环: 环境, 名称: str) -> int | None:
        if 节点 is None:
            return None
        值 = self._求值(节点, 环)
