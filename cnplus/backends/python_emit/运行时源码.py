...
def 取切片(对象, 起, 止, 步, 行, 列):
    """按 Python 的左闭右开/省略/负步长规则切片。"""
    ...
    def 整数或空(值, 名称):
        ...
        if isinstance(值, bool) or not isinstance(值, int):
            raise CNplus错误("CN0303", f"切片的{名称}必须是整数，这里是{类型名(值)}", 行, 列,
                          解释="[起:止:步长] 里要写位置或步幅，都必须用整数",
    ...
    步 = 整数或空(步, "步长")
    if 步 == 0:
        raise CNplus错误("CN0303", "切片步长不能是 0", 行, 列,
                      解释="步长是每次往前或往后走几格，写 0 就永远走不动",
                      提示="改成 1、2 或 -1 这类非零整数")
    return 对象[slice(起, 止, 步)]
