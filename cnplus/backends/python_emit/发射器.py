...
class 发射器:
    ...
    def _语句(self, s: 语句, 环名: str) -> None:
        ...
        if isinstance(s, 成员赋值语句):
            ...
            if s.复合 is not None:
                运名 = _二元函数[s.复合]
                self._行(
                    f"复合设成员({对象}, {s.属性!r}, lambda: {值}, {运名}, {行}, {列})", 行)
            else:
                self._行(f"设成员({对象}, {s.属性!r}, {值}, {行}, {列})", 行)
            return
        ...
        if isinstance(s, 索引赋值语句):
            ...
            值 = self._表达式(s.值, 环名)
            下标行, 下标列 = self._位置(s.下标.跨)
            if s.复合 is not None:
                运名 = _二元函数[s.复合]
                self._行(
                    f"复合设索引({对象}, {下标}, lambda: {值}, {运名}, {行}, {列}, "
                    f"{下标行}, {下标列})", 行)
            else:
                self._行(
                    f"设索引({对象}, {下标}, {值}, {行}, {列}, {下标行}, {下标列})", 行)
            return
    ...
    def _类(self, s: 类声明, 环名: str) -> None:
        ...
        方法体生成与普通函数一致（def 名_实现_序号(_环)），
        但「自己」由运行时在调用/实例化时注入，不在形参里。
        初始化块体存进方法表 '__初始化__' 槽位。
        """
        行, 列 = self._位置(s.跨)
        旧在函数内 = self.在函数内
        self.在函数内 = True
        ...
        # 方法表 dict 字面量 {名: 函数值, ...}
        对们 = ", ".join(
            f"{名!r}: 函数值({实现}, {环名}, {形参!r}, {'初始化' if 名 == '__初始化__' else 名!r}, [{默认}])"
            for 名, 实现, 形参, 默认 in 项们)
        表达 = "{" + 对们 + "}" if 对们 else "{}"
        # 初始化形参（实例化时自动转存字段的名字）
        初形参 = list(s.初始化.形参) if s.初始化 else []
        self._行(f"{环名}.声明({s.名!r}, 类值({s.名!r}, {表达}, {初形参!r}), {行}, {列})", 行)

    def _方法函数(self, m: 函数声明, 环名: str, 类名: str,
                内部名: str | None = None) -> tuple[str, str, list, str]:
        """生成一个方法体函数，返回 (名, 实现名, 形参, 默认串)。

        空方法体（省略块体的初始化）生成 pass，保证 Python 语法合法。
        """
        行, 列 = self._位置(m.跨)
    ...
    def _表达式(self, e: 表达式, 环名: str) -> str:
        ...
        if isinstance(e, 索引访问):
            ...
            下标行, 下标列 = self._位置(e.下标.跨)
            return f"取索引({对象}, {下标}, {行}, {列}, {下标行}, {下标列})"
        ...
        if isinstance(e, 调用表达式):
            ...
            return (f"调用({被调}, [{实参}], [{关键字}], "
                    f"{行}, {列}, {止行}, {止列})")
        if isinstance(e, 切片访问):
            对象 = self._表达式(e.对象, 环名)
            起 = self._表达式(e.起, 环名) if e.起 is not None else "None"
            止 = self._表达式(e.止, 环名) if e.止 is not None else "None"
            步长 = self._表达式(e.步长, 环名) if e.步长 is not None else "None"
            起位置 = repr(self._位置(e.起.跨)) if e.起 is not None else "None"
            止位置 = repr(self._位置(e.止.跨)) if e.止 is not None else "None"
            步长位置 = repr(self._位置(e.步长.跨)) if e.步长 is not None else "None"
            return (f"取切片({对象}, {起}, {止}, {步长}, {行}, {列}, "
                    f"{起位置}, {止位置}, {步长位置})")
