...
class 发射器:
    ...
    def _语句(self, s: 语句, 环名: str) -> None:
        ...
        if isinstance(s, 成员赋值语句):
            ...
            if s.复合 is not None:
                运名 = _二元函数[s.复合]
                self._行(
                    f"复合设成员({对象}, {js_str(s.属性)}, () => ({值}), {运名}, {行}, {列});", 行)
            else:
    ...
    def _表达式(self, e: 表达式, 环名: str) -> str:
        ...
        if isinstance(e, 二元表达式):
            # 与/或：短路 + 两侧强制布尔——必须用原生 &&/||
            # （函数调用会先求全部实参，短路就没了：BFS 里
            # 可走(点) 与 未访问[点] 的右侧在越界时会先炸）
            if e.运算 is 二元运算.与:
        ...
        if isinstance(e, 切片访问):
            ...
            步长 = self._表达式(e.步长, 环名) if e.步长 is not None else "null"
            起位置 = (f"[{e.起.跨.起.行}, {e.起.跨.起.列}]"
                    if e.起 is not None else "null")
            止位置 = (f"[{e.止.跨.起.行}, {e.止.跨.起.列}]"
                    if e.止 is not None else "null")
            步长位置 = (f"[{e.步长.跨.起.行}, {e.步长.跨.起.列}]"
                     if e.步长 is not None else "null")
