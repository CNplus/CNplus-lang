...
class CNplus错误 extends Error {
    constructor(码, 消息, 行, 列, 提示, 解释, 止行 = null, 止列 = null) {
        super(消息);
        this.码 = 码;
        this.消息 = 消息;
        this.行 = 行;
...
function 设索引(对象, 下标, 值, 行, 列, 下标行 = null, 下标列 = null) {
    if (Array.isArray(对象)) {
        const i = _序列下标(
            下标, 对象.length, 下标行 ?? 行, 下标列 ?? 列, true);
        对象[i] = 值;
        return;
    }
    if (对象 instanceof Map) {
...
function 复合设索引(
    对象, 下标, 求右侧, 运算, 行, 列, 下标行 = null, 下标列 = null
) {
    // 对象、下标已按左到右求值；先读旧值，再执行右侧，最后写回。
    const 旧 = 取索引(对象, 下标, 行, 列, 下标行, 下标列);
    const 右 = 求右侧();
    设索引(对象, 下标, 运算(旧, 右, 行, 列),
        行, 列, 下标行, 下标列);
}
...
function 实例化(类, 实参们, 关键字们, 行, 列, 止行 = null, 止列 = null) {
    const 实例 = new 实例值(类, new Map());
    const 初 = 类.方法表.get("__初始化__");
    if (初 === undefined) {
        if (实参们.length > 0 || 关键字们.length > 0) {
            throw new CNplus错误("CN0306",
                `${类.名} 没有写「初始化」，不需要参数，给了 ${实参们.length + 关键字们.length} 个`,
                行, 列,
                `直接写 ${类.名}()`,
                "类没定义初始化，造它的时候括号里不用填东西", 止行, 止列);
        }
        return 实例;
    }
    const 局部 = _绑定(初, 实参们, 关键字们, 行, 列, 止行, 止列);
...
function 点调用(全局, 对象, 方法名, 实参们, 关键字们, 行, 列,
    ...
    if ((Array.isArray(对象) || 对象 instanceof Map || typeof 对象 === "string")
        ...
        if (函数 !== undefined) {
            return _调用内置(
                函数, [对象, ...实参们], 关键字们, 行, 列, 止行, 止列);
        }
    }
    const 成员 = 取成员(对象, 方法名, 行, 列);
    return 调用(成员, 实参们, 关键字们, 行, 列, 止行, 止列);
}


// ==================== 调用 ====================

function _调用内置(被调, 实参们, 关键字们, 行, 列, 止行, 止列) {
    if (关键字们.length > 0) {
        throw new CNplus错误("CN0306",
            `内置函数 ${被调._内置名} 不接受「名字 = 值」参数`, 行, 列,
            "去掉参数名，按说明中的顺序传值",
            "内置函数只按位置接收参数", 止行, 止列);
    }
    return 被调._按位置调用(实参们, 行, 列, 止行, 止列);
}


function 调用(被调, 实参们, 关键字们, 行, 列, 止行 = null, 止列 = null) {
    if (被调 instanceof 类值) {
        return 实例化(被调, 实参们, 关键字们, 行, 列, 止行, 止列);
    }
    if (被调 instanceof 绑定方法) {
        const 局部 = _绑定(被调.函, 实参们, 关键字们, 行, 列, 止行, 止列);
        局部.声明("自己", 被调.实例);
        return 被调.函.实现(局部);
    }
    if (被调 instanceof 函数值) {
        const 局部 = _绑定(被调, 实参们, 关键字们, 行, 列, 止行, 止列);
        return 被调.实现(局部);
    }
    if (typeof 被调 === "function") {
        if (typeof 被调._内置名 === "string") {
            return _调用内置(
                被调, 实参们, 关键字们, 行, 列, 止行, 止列);
        }
        return 被调(...实参们);
    }
...
function _绑定(被调, 实参们, 关键字们, 行, 列, 止行 = null, 止列 = null) {
    const 形参 = 被调.形参们;
    const 默认 = 被调.默认们;
    if (实参们.length > 0 && 关键字们.length > 0) {
        throw new CNplus错误("CN0306",
            `${被调.名} 的一次调用不能混用位置参数和「名字 = 值」参数`, 行, 列,
            "统一改成一种写法",
            "自定义函数的一次调用只能全部按位置传值，或全部写成「名字 = 值」",
            止行, 止列);
    }
    if (实参们.length > 形参.length) {
        throw new CNplus错误("CN0306",
            ...
            行, 列,
            `参数有：${形参.join("、") || "无"}`,
            `你定义 ${被调.名} 时写了 ${形参.length} 个位置`, 止行, 止列);
    }
    const 已填 = new Map();
    ...
    for (const [名, 值] of 关键字们) {
        if (!形参.includes(名)) {
            throw new CNplus错误("CN0306",
                `${被调.名} 没有叫「${名}」的参数`, 行, 列,
                "检查参数名是不是写错了",
                `${被调.名} 的参数是：${形参.join("、") || "无"}`, 止行, 止列);
        }
        if (已填.has(名)) {
            throw new CNplus错误("CN0306", `参数「${名}」给了两次`, 行, 列,
                "删掉重复的那个", "一个参数只能赋值一次", 止行, 止列);
        }
        已填.set(名, 值);
    }
...
function _极值(取最大, ...参数) {
    ...
    if (是数(第一项)) {
        if (值们.some(项 => !是数(项))) {
            throw new CNplus错误("CN0303", "最大/最小的项目必须全是数值或全是文字", 0, 0);
        }
    } else if (typeof 第一项 === "string") {
        if (值们.some(项 => typeof 项 !== "string")) {
            throw new CNplus错误("CN0303", "最大/最小的项目必须全是数值或全是文字", 0, 0);
        }
    } else {
        throw new CNplus错误("CN0303", "最大/最小的项目必须全是数值或全是文字", 0, 0);
...
function _倒序(值, 行, 列) {
    if (Array.isArray(值)) return [...值].reverse();
    if (typeof 值 === "string") return [...值].reverse().join("");
    throw new CNplus错误("CN0303", "倒序只接受列表或文字", 行, 列,
        '传入列表或文字；文字也可以写 "甲😀乙"[::-1]',
        "倒序会返回和输入相同类型的新值");
}


function _检查内置参数数量(名, 最少参数, 最多参数, 数量, 行, 列,
        止行 = null, 止列 = null) {
    if (!(数量 < 最少参数 || (最多参数 >= 0 && 数量 > 最多参数))) return;
    let 消息;
    let 解释;
    let 提示;
    if (最少参数 === 最多参数) {
        消息 = `${名} 需要 ${最少参数} 个参数，给了 ${数量} 个`;
        解释 = `括号里该填几个东西是固定的，${名} 要 ${最少参数} 个`;
        提示 = "数一数括号里用逗号隔开的项";
    } else if (最少参数 === 0 && 最多参数 === 1) {
        消息 = `${名} 最多 1 个参数（提示语），给了 ${数量} 个`;
        解释 = `${名}() 直接问，${名}("提示") 带提示语`;
        提示 = `例如：${名}("请输入名字：")`;
    } else if (最少参数 === 1 && 最多参数 === 2) {
        消息 = `${名} 需要 1 或 2 个参数，给了 ${数量} 个`;
        解释 = `${名} 的第二个参数是可选的`;
        提示 = "检查括号里用逗号隔开的项";
    } else if (最少参数 === 1 && 最多参数 === 3) {
        消息 = `${名} 需要 1 到 3 个参数，给了 ${数量} 个`;
        解释 = `${名} 后两个参数是可选的`;
        提示 = "例如：范围(5)、范围(1, 6)、范围(1, 10, 2)";
    } else {
        消息 = `${名} 的参数数量不对，给了 ${数量} 个`;
        解释 = "括号里的参数数量不符合这个内置函数的要求";
        提示 = "检查括号里用逗号隔开的项";
    }
    throw new CNplus错误("CN0306", 消息, 行, 列, 提示, 解释, 止行, 止列);
}


function _检查内置类型(名, 实参们, 行, 列) {
    if (实参们.length === 0) return;
    const 第一个 = 实参们[0];
    const 类型错误 = (消息) => { throw new CNplus错误("CN0303", 消息, 行, 列); };
    if (名 === "绝对值") {
        if (!是数(第一个)) 类型错误("绝对值只能用于数字");
        return;
    }
    if (名 === "长度") {
        if (!(Array.isArray(第一个) || 第一个 instanceof Map || typeof 第一个 === "string"))
            类型错误("长度只能用于列表、字典或文字");
        return;
    }
    if (名 === "求和") {
        if (!Array.isArray(第一个)) 类型错误("求和只能用于列表");
        return;
    }
    if (["追加", "插入", "移除", "弹出", "排序", "连接"].includes(名)) {
        if (!Array.isArray(第一个)) 类型错误(`${名}只能用于列表`);
        if (名 === "排序" && 第一个.length > 0) {
            const 第一项 = 第一个[0];
            const 合法 = 是数(第一项) ? 第一个.every(项 => 是数(项)) :
                typeof 第一项 === "string" ? 第一个.every(项 => typeof 项 === "string") : false;
            if (!合法) 类型错误("排序的项目必须全是数值或全是文字");
        }
        if (名 === "连接" && typeof 实参们[1] !== "string")
            类型错误("连接的分隔内容必须是文字");
        return;
    }
    if (["所有标签", "所有值", "有标签", "删标签"].includes(名)) {
        if (!(第一个 instanceof Map)) 类型错误(`${名}只能用于字典`);
        return;
    }
    if (名 === "包含") {
        if (!(Array.isArray(第一个) || 第一个 instanceof Map || typeof 第一个 === "string"))
            类型错误("包含只能用于列表、字典或文字");
        if (typeof 第一个 === "string" && typeof 实参们[1] !== "string")
            类型错误("文字包含的内容必须是文字");
        return;
    }
    if (["分割", "替换", "查找", "去空白", "大写", "小写", "开头是", "结尾是"].includes(名)) {
        if (typeof 第一个 !== "string") 类型错误(`${名}只能用于文字`);
        if (名 === "分割" && 实参们.length === 2 && typeof 实参们[1] !== "string")
            类型错误("分割的分隔内容必须是文字");
        if (名 === "替换" && typeof 实参们[1] !== "string")
            类型错误("替换的原内容必须是文字");
        if (名 === "替换" && typeof 实参们[2] !== "string")
            类型错误("替换的新内容必须是文字");
        if (名 === "查找" && typeof 实参们[1] !== "string")
            类型错误("查找的内容必须是文字");
        if (名 === "开头是" && typeof 实参们[1] !== "string")
            类型错误("开头是的前缀必须是文字");
        if (名 === "结尾是" && typeof 实参们[1] !== "string")
            类型错误("结尾是的后缀必须是文字");
    }
}


function _语言内置(名, 最少参数, 最多参数, 实现) {
    const 包装 = (...实参们) => {
        _检查内置参数数量(名, 最少参数, 最多参数, 实参们.length, 0, 0);
        _检查内置类型(名, 实参们, 0, 0);
        return 实现(...实参们);
    };
    包装._内置名 = 名;
    包装._按位置调用 = (实参们, 行, 列, 止行 = null, 止列 = null) => {
        _检查内置参数数量(
            名, 最少参数, 最多参数, 实参们.length, 行, 列, 止行, 止列);
        _检查内置类型(名, 实参们, 行, 列);
        try {
            if (typeof 实现._按位置调用 === "function") {
                return 实现._按位置调用(实参们, 行, 列);
            }
            return 实现(...实参们);
        } catch (错误) {
            if (错误 instanceof CNplus错误 && 错误.行 === 0 && 错误.列 === 0) {
                错误.行 = 行;
                错误.列 = 列;
            }
            throw 错误;
...
for (const [名, 实现] of Object.entries(内置们)) {
    const [最少参数, 最多参数] = _内置参数范围[名];
    内置们[名] = _语言内置(名, 最少参数, 最多参数, 实现);
}
