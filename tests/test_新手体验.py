...
def test_示例不用多余的引号转义(文件: Path):
    ...
    这是 v0.6.1 定下的写法规范：单引号在双引号格式串里不需要转义。
    示例是给人看的，写法不一致会误导读者照抄多余的转义。
    """
    码 = 文件.read_text(encoding="utf-8")
    assert "\\'" not in 码, (
        f"{文件.name} 里有多余的 \\' 转义。"
...
def test_未声明_忘加引号的两案提示():
    """`设 名字 = 小明`：最常见的真实意图是想写字符串但忘了双引号。

    提示必须同时给出两种可能（加引号 / 先声明），且不得再给
    「是不是想写小数/小写」这类中文模糊误猜（小明/小写 距离同为 1，
    数学上无法区分，猜了就是误导）。
    """
    _, 袋, _ = 跑("设 名字 = 小明\n打印(名字)")
    提示们 = " ".join(d.提示 or "" for d in 袋)
    assert "双引号" in 提示们, "应提示可能是忘了加引号"
    assert "小明" in 提示们 and "建立" in 提示们, "应同时提示先声明的可能"
    assert "是不是想写" not in 提示们, "中文模糊近似已废除"


def test_未声明_手滑多打字仍能猜中():
    """`年龄x`（已声明 年龄）：名字是候选的超集扩展 —— 强信号，应猜中。"""
    _, 袋, _ = 跑("设 年龄 = 18\n打印(年龄x)")
    提示们 = " ".join(d.提示 or "" for d in 袋)
    assert "年龄" in 提示们 and "是不是想写" in 提示们


def test_未声明_英文拼写错误仍能猜中():
    """`helo`（已声明 hello）：纯 ASCII 距离 1 —— 英文拼写错误，应猜中。"""
    _, 袋, _ = 跑("设 hello = 1\n打印(helo)")
    提示们 = " ".join(d.提示 or "" for d in 袋)
    assert "hello" in 提示们 and "是不是想写" in 提示们
